...
import type { CompareMode, Nudge, ReferenceView } from '../src/model';
...
/**
 * Wraps the 3D viewport and puts the reference image either over it or beside it.
 *
 ...
 */
export default function CompareStage({ view, mode, nudge, children }: Props) {
  ...
  if (mode === 'overlay') {
    return (
      <div className="stage" ref={stageRef}>
        <div className="overlay-fit" style={aspect ? { aspectRatio: String(aspect) } : undefined}>
          {children}
          <div
  ...
  return (
    <div className="stage split" ref={stageRef}>
      ...
      />
      <div className="split-pane model" style={{ width: `${(1 - split) * 100}%` }}>
        {children}
      </div>
    </div>
  );

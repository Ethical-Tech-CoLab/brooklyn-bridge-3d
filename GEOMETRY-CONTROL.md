...
| CTL-110 | approach_arcade_bay | 30 | ft | none | D | PLACEHOLDER, and **constrained rather than free**. Longitudinal pier-to-pier bay of the approach arcade. Was 100 ft, invented when the model drew slender bents; that value is geometrically **impossible** for the arcade HAER NY-18-64 shows, because a two-centred pointed arch requires a rise of at least half its clear span and a 100 ft bay would need a rise of over 46 ft. CHK-009 now asserts that constraint on every build, so this number cannot drift back to a value that draws an arch which cannot exist. The **extent** the arcade covers is grade `A` (CTL-005, CTL-006); the rhythm within it is still invented. See OQ-007. |
| CTL-111 | approach_pier_width_x | 8 | ft | none | D | PLACEHOLDER. Longitudinal width of an arcade pier. See OQ-007. |
| CTL-112 | approach_girder_depth | 10 | ft | none | D | PLACEHOLDER. See OQ-007. |
| CTL-113 | approach_arch_rise | 22 | ft | none | D | PLACEHOLDER, and a *new* invention introduced deliberately. HAER NY-18-64 settles that the approach is a masonry arcade of pointed arches, not a line of bents — but a pointed arch cannot be drawn without a rise, and no read source gives one. Rather than leave the model showing an object of the wrong kind because the right kind needs one more number, the arch is built and the number is declared an invention. Chosen only so the arch springs below the deck soffit at every station along both approaches; it is excluded from every dimension callout, and the whole arcade is rendered as a placeholder. See OQ-007. |
| CTL-114 | approach_pier_depth_y | 26 | ft | none | D | PLACEHOLDER. Transverse depth of an arcade pier. The arcade carries the full deck width, so this is bounded above by `deck_width` (85 ft, grade A) and is nowhere near it in NY-18-64, where the piers read as substantially deeper than they are wide. Invented; see OQ-007. |
...
| OQ-016 | How is the tower masonry above the saddle actually arranged, and how wide is the opening the cables pass through? | The width of the cornice opening, and therefore how much of the tower top reads as solid | **ANSWERED IN KIND, AND THE MODEL IS WRONG.** HAER **NY-18-41** (LOC 120531) photographs the two centre cables from inside the Brooklyn tower "extending out of masonry openings": each cable passes through its **own discrete tunnel** in the granite, with continuous masonry over it, onto a cast-iron saddle standing in an enclosed chamber — **NY-18-40** (LOC 120530) shows the saddles themselves under a brick vault. The model instead opens the masonry as a single corridor spanning all four cables, which is the wrong *kind* of arrangement, not merely the wrong width. See CONF-009. The opening's dimensions remain unmeasured and a photograph cannot supply them; that part still needs NYCDOT drawings. |
...
| OQ-007 | What carries the approach viaducts, and at what spacing? | CTL-110, CTL-111, CTL-112 | **The model's supports are the wrong *kind* of thing, not merely the wrong size.** SRC-004 describes brick piers and arches, and SRC-016's modern photography shows a masonry arcade carrying the approach, not a line of slender bents. **Now settled in kind by HAER NY-18-64** (LOC 120554), a square elevation of the Brooklyn approach: a continuous arcade of pointed arches springing from rectangular granite piers, carrying a balustraded parapet. The model draws slender bents, which is simply not this object. The *spacing* is still a dimension and still needs NYCDOT drawings — a photograph cannot supply it. The SRC-018 crowd campaign returned zero `arcade` frames while this plate sat unread in a registered source; see the note on SRC-007 in the register. |

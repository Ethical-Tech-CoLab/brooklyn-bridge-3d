...
| SRC-007 | HAER No. NY-18 photographs (77 b/w, 8 colour transparencies) | [loc.gov/pictures/collection/hh/item/ny1234](https://www.loc.gov/pictures/collection/hh/item/ny1234/) · **index: [sources/haer-ny18-photo-index.json](sources/haer-ny18-photo-index.json)** | Archival photographs | U.S. Government work | **read** — all 77 indexed with captions, LOC digital IDs and image URLs; four plates retrieved and examined | B for arrangement and material. **Never for a dimension.** | Tier B detail evidence. **NY-18-64** settles OQ-007 in kind (the approach is a masonry arcade of pointed arches on granite piers, not bents). **NY-18-40** and **NY-18-41** settle OQ-016 in kind and show the model is wrong: each cable passes through its own masonry opening onto a saddle in an enclosed chamber, not over an open corridor — see CONF-009. **NY-18-38** shows the stay fan leaving the tower across a spread of heights, not from one node. None of these carries a dimension. |
...
| SRC-007, the HAER photographs | **This row was true and is now the most expensive mistake in the project.** It said plainly that the plates had not been read — and a 272-image crowd campaign was then run to answer OQ-007 and OQ-016, both of which these plates answer. NY-18-64 is a square elevation of the approach arcade; NY-18-40 and NY-18-41 photograph the cable saddles and the masonry openings from inside the Brooklyn tower. All 77 are now indexed in [sources/haer-ny18-photo-index.json](sources/haer-ny18-photo-index.json), with captions, LOC digital IDs and direct image URLs. **Ask the sources you already hold before you ask the crowd.** |
...
| **CONF-008 — deck elevation at the towers** | SRC-002's tower detail annotates `110' CLEARANCE TO WATERLINE`. Scaling SRC-002's own elevation gives the deck underside at about **117 ft** at the Manhattan tower and **124 ft** at the Brooklyn tower. | **open** | Both figures come from the same sheet. The annotated 110 ft is an explicit statement and wins under the rule that an explicit statement beats arithmetic, so CTL-011 is unchanged and the model still uses it. But the scaled reading is recorded because the same scaling, on the same sheet, reproduces the drawing's stated 135 ft midspan clearance to within 1.1 ft — so it is not obviously wrong. The consequence is that the model's deck may be cambered more steeply than the real one. See OQ-017. |
| **CONF-009 — how the cables pass the tower top** | The model builds an open corridor: cornice masses outboard of the outer cables, with the masonry stepped down between them so all four cables ride saddles in one continuous gap. HAER **NY-18-41** photographs the two centre cables "extending out of masonry openings" — each cable in its own tunnel through the granite, masonry continuous above it — and **NY-18-40** shows the cast-iron saddles standing in an enclosed, brick-vaulted chamber. | **open — the model is wrong and the correction is not yet buildable** | The photographs settle the *kind* of arrangement and the model does not match it. They cannot settle the opening's width, the chamber's extent, or the saddle plinth height, and inventing those to make the geometry "look right" would replace a visible error with an invisible one. The model therefore keeps its present tower top, now labelled as known-wrong via OQ-016, until NYCDOT drawings or SRC-002's tower sheet supply the dimensions. **Preferring a wrong shape that is registered as wrong over a plausible shape that is unregistered is the whole point of this repository.** |
